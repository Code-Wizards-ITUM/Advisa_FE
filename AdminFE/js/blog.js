...
async function fetchBlogs() {
  try {
    const response = await fetch(`${API_BASE_URL}admin/getBlogs`, {
      method: "GET",
...
async function removeBlog(index) {
  try {
    const blog = BLOGS[index];
    const confirmation = confirm("Want to remove this blog post?");
    if (confirmation) {
      const response = await fetch(`${API_BASE_URL}admin/deleteBlog`, {
        method: "DELETE",
